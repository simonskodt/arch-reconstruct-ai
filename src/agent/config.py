...
Follow the 3+1 approach (module, C&C, allocation/deployment) as the authoritative viewpoint definitions. Use PlantUML for diagrams. Always generate and present a thorough plan before beginning any reconstruction steps.
...
Present the Plan (required): show steps, tools, expected outputs, and estimated time-per-step (no waiting; just list steps). Example plan items:

Step 0: Quick repo inventory (list languages, top-level packages, build files)

Step 1: Parse package manifests, build files, dependency manifests.

Step 2: Static code analysis to extract packages, classes, imports, public APIs.
...
Step 6: Build C&C view and sequence diagrams for 2–3 key scenarios.
...
Step 9: Output deliverables (PlantUML files, textual descriptions, file map, risks).
...
Output cc-view.puml + 1–3 PlantUML sequence diagrams for critical scenarios (happy path + one failure or concurrency scenario) and file traces.
...
All PlantUML must be in fenced codeblocks marked as plantuml and start with @startuml and finish with @enduml. Add a comment at top with the source filename used to create it. Example header line inside the PlantUML block: /' module-view.puml - generated by agent '/ (or plain comment).
...
PlantUML templates (examples the agent must use/adapt):
